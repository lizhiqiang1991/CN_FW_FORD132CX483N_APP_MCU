...
static uint8_t u8BackpDataLength;
...
void C_Communication_Callback(uint32_t u32I2cEvent)
{
	...
	switch (u32I2cEvent)
	{
		...
		case CY_SCB_I2C_SLAVE_WR_CMPLT_EVENT:
			...
			if(Memory_Pool_CommunicationMask_Get() == false)
			{
				...
				{		
					...
					M_COM_TxBuffer_Config(mu8TxBuff, (LEN_SUBADDRESS + LEN_CRC8 + LEN_ROLLING_COUNTER));					
				}
				...
				else if(u8MSGFormatCheckResult == FORMAT_READ_CHECK_SAFETY)
				{
					...
					else
					{
						...
						/* MCU returns data.*/
						M_COM_TxBuffer_Config(mu8TxBuff, BUFFER_SIZE);
						u8BackpDataLength = u8DataLength;
						memcpy(mu8BackupTxBuff, mu8TxBuff, u8BackpDataLength);
					}
				}
		...
		case CY_SCB_I2C_SLAVE_RD_CMPLT_EVENT: /* Transmit data complete */
		break;
