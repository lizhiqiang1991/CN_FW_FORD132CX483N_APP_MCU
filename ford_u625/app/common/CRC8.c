...
#include "Crc8.h"
 ...
 static uint8_t u8Crc = 0U;
/******************************************************************************
* Name         :crc8_table
* Called by    :Calculation CRC8
* Return code  :N/A
* Description  :CRC8 fast algorithm table

Algorithm          	Poly    Init    RefIn 	RefOut 	XorOut     
 CRC-8	            0x07	0x00	false	false	0x00

******************************************************************************/
// Polynomial = 0x7	(0x83; 0x107)

// CRC Lookup Table for #0x83 = x^8 +x^2 +x +1 (0x107) <=> (0xe0; 0x1c1) 
const uint8_t mu8CRC_table_0x83[256U] =
...
/******************************************************************************
* Name         :CRC8_Calculation
* Called by    :IC Message
* Return code  :crc result
* Description  :Calculation CRC8
******************************************************************************/
uint8_t CRC8_Calculation(uint8_t *pu8data,uint8_t u8Length)
{
    u8Crc = 5U; // CRC is non-zero CRC when all data is zero 
    uint8_t u8CrcTemp= 0U;
    uint8_t u8Counter = 0U;

    for(u8Counter = 0U; u8Counter < u8Length; u8Counter++)
    {  
        // XOR datat byte into CRC 
        u8CrcTemp = (pu8data[u8Counter] ^ u8Crc);
        // fetch CRC value from table 
        u8Crc = mu8CRC_table_0x83[u8CrcTemp];
    }
